...
var D3Pie = (function (selector, complete, progress) {
  ...
  var data = [{age: 'complete', population: complete}, {age: 'progress', population: progress}];
  var g = svg.selectAll(".arc").data(pie(data)).enter().append("g").attr("class", "arc");

  g.append("path").attr("d", arc).style("fill", function(d) {
    return d.data.age == 'complete' ? '#009933' : '#993333';
  });

  g.append("text").attr("transform", function(d) { return "translate(" + labelArc.centroid(d) + ")"; }).attr("dy", ".35em").text(function(d) {
    return d.data.age;
  });
  return "";
});